...
- `FUND2_DEALS` (between `/* @fund2-data-start */` and `/* @fund2-data-end */` sentinels) — Nectar Fund II's per-deal slice (57 positions). Generated by `build_fund2.py` from the latest `Borrowing_Base_Tape*.csv`; do not hand-edit. Wraps into `FUND2 = { ids, byId }` and a virtual `RAW_PLUS_FUND2` that adds synthetic rows for Fund II positions not in `RAW`. When the **Fund** filter dropdown is set to "Nectar Fund II", `metricFor(d, field)` swaps in fund-slice values (outstanding, monthly payment, advance × exposure%) and the table shows an extra "Fund II Exposure" column. To refresh from a new tape: `python3 build_fund2.py "<path/to/Borrowing_Base_Tape.csv>"`.
